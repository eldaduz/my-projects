...
describe('Gemini adapter reliability (ATP-69)', () => {
  ...
  test('retries once on a 5xx ApiError, then succeeds on the retry', async () => {
    ...
    const adapter = createGeminiAdapter({ client: { models: { generateContent } }, timeoutMs: 1000 });
  ...
  test('does not retry a 429 and throws RATE_LIMITED', async () => {
    ...
    await expect(adapter.generateItinerary(context)).rejects.toMatchObject({ code: 'RATE_LIMITED' });
  ...
  test('does not retry other 4xx errors and throws PROVIDER_REJECTED', async () => {
    ...
    await expect(adapter.generateItinerary(context)).rejects.toMatchObject({ code: 'PROVIDER_REJECTED' });
    expect(generateContent).toHaveBeenCalledTimes(1);
  });
  ...
  test('missing provider text is a non-retryable PROVIDER_REJECTED error', async () => {
    ...
    await expect(adapter.generateItinerary({})).rejects.toMatchObject({ code: 'PROVIDER_REJECTED' });
    expect(generateContent).toHaveBeenCalledTimes(2);
  });
});
